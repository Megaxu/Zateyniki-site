<!DOCTYPE php>
<php lang="en">

    <head>
        <meta charset="UTF-8">
        <title>Мафия</title>
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta property="og:title" content="Мафия">
        <meta property="og:site_name" content="Затейники-СПБ оргнанизация праздников в СПБ">
        <meta property="og:url" content="mafia-show">
        <meta property="og:image" content="images/mafia-show-preview.jpg">
        <link rel="stylesheet" href="css/normalize.css">
        <link rel="stylesheet" href="css/style.css">
        <link rel="stylesheet" href="css/slider.css">
        <link rel="stylesheet" href="css/mafia-show.css">
        <!-- Yandex.Metrika counter -->
        <script type="text/javascript">
            (function(m, e, t, r, i, k, a) {
                m[i] = m[i] || function() {
                    (m[i].a = m[i].a || []).push(arguments)
                };
                m[i].l = 1 * new Date();
                k = e.createElement(t), a = e.getElementsByTagName(t)[0], k.async = 1, k.src = r, a.parentNode.insertBefore(k, a)
            })
            (window, document, "script", "https://mc.yandex.ru/metrika/tag.js", "ym");

            ym(57696484, "init", {
                clickmap: true,
                trackLinks: true,
                accurateTrackBounce: true,
                webvisor: true
            });
        </script>
        <noscript>
            <div><img src="https://mc.yandex.ru/watch/57696484" style="position:absolute; left:-9999px;" alt="" /></div>
        </noscript>
        <!-- /Yandex.Metrika counter -->
    </head>

    <body>

        <?php include("blocks/navigation.php"); ?>

        <div class="banner">
            <div class="wrapper">
                <div class="box">
                    <div class="center">
                        <h1>Мафия</h1>
                    </div>
                    <div class="box-info">
                        <div class="text-block">
                            <p>"Мафия бессмертна!" или стоит проверить эту известную фразу? Эта игра пользуется большой популярностью как среди детей, так и среди взрослых. В самом названии заложена ее главная идея - это интересная, веселая, полезная и захватывающая игра с детективным сюжетом.Отличительная особенность игры - она не надоедает никогда! Потому-что каждая новая игра - это новая увлекательная "жизнь" персонажа.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="main">
            <div class="wrapper">
                <div class="center">
                    <div class="box-wait-you">
                        <h2 class="h2-fiol">Что вас ждет?</h2>
                        <div class="box-info">
                            <img src="images/best-mafia.jpg" alt="fixies-foto" width="700px" height="470px">
                            <div class="text-block-wait-you">
                                <p>1. Два ведущих;</p>
                                <p>2. Программа на 1 или 2 часа;</p>
                                <p>3. Детский и взрослый сценарий;</p>
                                <p>4. Памятные призы каждому гостю;</p>
                                <p>5. Необычный современный формат.</p>
                            </div>
                        </div>
                    </div>
                    <div class="box-galery-mafia">
                        <div class="wrapper">
                            <div class="center">
                                <h2 class="h2-fiol">Галерея</h2>
                            </div>
                            <div id="slider-wrap">
                                <div id="slider">
                                    <div class="slide">
                                        <img src="images/mafia_1.jpg" width="690" height="470">
                                    </div>
                                    <div class="slide">
                                        <img src="images/mafia_2.jpg" width="690" height="470">
                                    </div>
                                    <div class="slide">
                                        <img src="images/mafia_3.jpg" width="690" height="470">
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <?php include("blocks/footer.php"); ?>

            <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js"></script>
            <script src="js/slider.js"></script>

    </body>

</php>